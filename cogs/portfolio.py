    ...
    async def _process_action(self, interaction, action, channel, owner, current_rank, current_tier, pinned_by):
        try:
            if action == "rank_up":
                ...
                await db.update_portfolio_pinned(channel.id, interaction.user.id)
                await refresh_portfolio_embed(channel)
                await interaction.followup.send("✅ Вы закрепились за портфелем.", ephemeral=True)

            elif action == "unpin":
                await db.update_portfolio_pinned(channel.id, None)
                await refresh_portfolio_embed(channel)
...
class GreenRequestModal(Modal, title="Запрос грина"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        req_id = await db.add_green_request(interaction.user.id, amount, level, self.channel_id)
        ...
        await db.update_green_request_message(req_id, msg.id)
...
class GreenRequestView(View):
    ...
    async def grant_green(self, interaction: discord.Interaction):
        ...
        req_data = await db.get_green_request(self.req_id)
        if not req_data:
            return await interaction.followup.send("❌ Запрос не найден.", ephemeral=True)

        user_id, amount, level, _ = req_data

        await db.update_green_request_status(self.req_id, 'granted', interaction.user.id)
