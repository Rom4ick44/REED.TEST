...
class VacationModal(Modal, title="Уход в отпуск"):
    def __init__(self, bot):
        super().__init__()
        self.bot = bot
        self.add_item(TextInput(label="Длительность", placeholder="например: 2 недели, 10 дней, 1 месяц", required=True, max_length=50))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await db.add_vacation(user_id, now, duration, reason, interaction.channel_id)
...
class Vacation(commands.Cog):
    ...
    async def setup_vacation_panel(self, ctx):
        ...
        if not channel:
            return await ctx.send("❌ Канал для панели отпусков не найден.")
        embed = discord.Embed(
